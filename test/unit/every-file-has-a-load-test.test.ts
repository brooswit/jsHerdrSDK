...
describe("every source file has a load test (bun's coverage only sees files a test imports)", () => {
  const files = loadableFiles(ROOT);
  test("there are source files to check", () => { expect(files.length).toBeGreaterThan(30); });
  for (const f of files) {
    test(`${f} → ${loadTestPathFor(f)}`, () => {
      ...
      expect(readFileSync(p, "utf8")).toBe(loadTestSource(f)); // and it is the CURRENT generated form, not a stale hand edit
    });
  }
  test("no orphan load tests for deleted sources", () => {
